...
create unique index if not exists inboxes_user_provider_email_key
  on public.inboxes (user_id, provider, email_address);
...
begin
  if not exists (
    ...
    where schemaname = 'public'
      ...
      and policyname = 'users_can_read_own_inboxes'
  ) then
    create policy users_can_read_own_inboxes
      on public.inboxes
      for select
      using (
        exists (
          select 1
          from public.users
          where users.id = inboxes.user_id
