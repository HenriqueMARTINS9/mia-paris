        ...
        {requests.length > 0 ? (
          requests.map((request) => (
            <Link
              ...
              className="block rounded-2xl border border-white/70 bg-white/70 p-4 transition hover:border-primary/20 hover:bg-white/90"
            >
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <div className="text-right text-sm text-muted-foreground">
                  ...
                  <p className="mt-2">
                    {request.dueAt ? formatDateTime(request.dueAt) : "Sans date"}
                  </p>
                  <p className="mt-1">{request.owner}</p>
                </div>
              </div>
            </Link>
          ))
